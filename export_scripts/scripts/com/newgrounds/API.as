package com.newgrounds
{
   ...
   public class API
   {
      ...
      {
         var _loc1_:String = GATEWAY_URL + "?tracker_id=" + _movieId + "&command_id=" + getCommandID("loadOfficalVersion") + "&seed=" + Math.random();
         if(debugMode)
         {
            _loc1_ += "&debug=1";
         }
      ...
      public static function connect(param1:LoaderInfo, param2:String, param3:String = null) : void
      {
         ...
         {
            if(Boolean(flashVars.NewgroundsAPI_UserName))
            {
               user_name = flashVars.NewgroundsAPI_UserName;
            }
            if(Boolean(flashVars.NewgroundsAPI_UserID))
            {
               user_id = flashVars.NewgroundsAPI_UserID;
            }
            if(Boolean(flashVars.NewgroundsAPI_PublisherID))
            {
            ...
            if(Boolean(flashVars.NewgroundsAPI_UserpageFormat))
            {
               _userpageFormat = flashVars.NewgroundsAPI_UserpageFormat;
            }
            if(Boolean(flashVars.NewgroundsAPI_SessionID))
            {
               session_id = flashVars.NewgroundsAPI_SessionID;
            }
            if(Boolean(flashVars.NewgroundsAPI_SaveGroupID))
            {
               _saveGroupId = flashVars.NewgroundsAPI_SaveGroupID;
            }
            if(Boolean(flashVars.NewgroundsAPI_SaveFileID))
            {
               _saveFileId = flashVars.NewgroundsAPI_SaveFileID;
            }
            if(Boolean(flashVars.NewgroundsAPI_SandboxID))
            {
      ...
      {
         return compressHex(RC4.encrypt(JSON.encode(param1),_encryptionKey));
      }
      
      public static function decodeData(param1:String) : *
      {
         return JSON.decode(RC4.decrypt(uncompressHex(param1),_encryptionKey));
      }
      ...
      public static function executeSaveQuery(param1:SaveGroupQuery) : void
      {
         sendCommand("lookupSaveFiles",{
            ...
            "query":JSON.encode(param1.toObject())
         },false,null,param1);
      }
      ...
      public static function checkFilePrivledges(param1:SaveFile) : void
      {
         sendCommand("checkFilePrivs",{
            ...
            "user_id":(Boolean(user_id) ? user_id : 0),
            "publisher_id":publisher_id
         });
      }
      ...
      {
         var _loc2_:Object = JSON.decode(RC4.decrypt(param1,_encryptionKey));
         ...
         switch(getCommandName(param1.command_id))
         {
            case "connectMovie":
               ...
               if(Boolean(param1.success))
               {
                  ...
                  if(Boolean(param1.movie_version))
                  {
                  ...
                  if(Boolean(param1.request_portal_url))
                  {
            ...
            case "preloadSettings":
               ...
               if(Boolean(param1.save_groups))
               {
                  populateSaveGroups(param1.save_groups);
               }
               if(Boolean(param1.save_file_path))
               {
                  _saveFilePath = param1.save_file_path + "/";
               }
               if(Boolean(param1.image_file_path))
               {
                  _imageFilePath = param1.image_file_path + "/";
               }
               if(Boolean(param1.score_boards))
               {
            ...
            case "loadSaveFile":
               if(Boolean(param1.success))
               {
                  ...
                  if(Boolean(param1.get_contents))
                  {
            ...
            case "rateSaveFile":
               ...
               param1.file = _loc15_;
               if(Boolean(param1.already_voted))
               {
      ...
      public static function sendSecureCommand(param1:String, param2:Object, param3:Object = null, param4:Object = null, param5:* = null) : void
      {
         ...
         var _loc9_:String = RC4.encrypt(JSON.encode(param2),_encryptionKey);
      ...
      private static function onCommandComplete(param1:Event) : void
      {
         ...
         if(!response || !response.success)
         {
            ...
            if(Boolean(response.command_id))
            {
               doEvent(response);
            }
         }
